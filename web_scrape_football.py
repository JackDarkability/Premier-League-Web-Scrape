...
def update_with_detailed_stats(results_of_matches):
    ...
    for match in results_of_matches:
        # Get dictionary of the detailed stats from the match page.

        detailed_stats = get_detailed_match_data(
            driver, match["match_id"], cookies_accepted
        )
...
if __name__ == "__main__":
    ...
    scrape_football_data(clubs=["-1"], output_file="football_matches_with_ID.csv")
    calculate_elo_and_save_to_file("football_matches_with_ID.csv", "home_team", "away_team")


    # Get detailed stats for each match (VERY SLOW AND LONG)
    '''
    ...
    results_of_matches.to_csv("football_matches_with_ID.csv", index=False)
    '''
